...
const std::unordered_set<std::string> FabricRichParser::BLOCK_LEVEL_TAGS = {
...
const std::unordered_set<std::string> FabricRichParser::INLINE_FORMATTING_TAGS = {
...
bool FabricRichParser::isBlockLevelTag(const std::string& tag) {
  return BLOCK_LEVEL_TAGS.find(tag) != BLOCK_LEVEL_TAGS.end();
}

bool FabricRichParser::isInlineFormattingTag(const std::string& tag) {
  return INLINE_FORMATTING_TAGS.find(tag) != INLINE_FORMATTING_TAGS.end();
}

Float FabricRichParser::getHeadingScale(const std::string& tag) {
...
int32_t FabricRichParser::parseHexColor(const std::string& colorStr) {
...
std::string FabricRichParser::getStringValueFromStyleObj(
...
Float FabricRichParser::getNumericValueFromStyleObj(
    const std::string& styleObj,
    const std::string& key) {
  std::string searchKey = "\"" + key + "\"";
  size_t keyPos = styleObj.find(searchKey);
...
FabricRichTagStyle FabricRichParser::getStyleFromTagStyles(
...
bool FabricRichParser::isParagraphBreak(const std::string& text) {
  for (char c : text) {
...
std::string FabricRichParser::normalizeSegmentText(
