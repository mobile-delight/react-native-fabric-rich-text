#import "FabricRichParsingUtils.h"

@implementation FabricRichParsingUtils

static dispatch_queue_t sSharedParsingQueue;

+ (void)initialize {
    if (self == [FabricRichParsingUtils class]) {
...
+ (void)parseHTML:(NSString *)html
    ...
    dispatch_async(sSharedParsingQueue, ^{
        ...
        if (data) {
            ...
            if (error) {
                NSLog(@"FabricRichParsingUtils: HTML parsing failed: %@", error.localizedDescription);
                attributedText = nil;
            }
        }
